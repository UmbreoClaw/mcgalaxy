    ...
    public sealed class CmdSummon : Command2 {
            ...
            if (!message.CaselessEq("all")) {
                ...
                foreach (Player target in players) {
                    if (target.level == p.level && target != p && data.Rank > target.Rank) {
                        ...
                        target.Message(Locale.Get("summon.summoned_by", target), target.FormatNick(p));
                    }
                }
                Chat.MessageFromLevel(p, Locale.Get("summon.summoned_everyone"));
            }
        }
        ...
        static void SummonPlayer(Player p, string message, CommandData data) {
            ...
            if (p.level != target.level) {
                ...
                p.Message(Locale.Get("summon.moving", p), p.FormatNick(target));
        ...
        static bool CheckVisitPerm(Player p, Player target, bool confirmed) {
            ...
            if (result == AccessResult.Blacklisted) {
                p.Message(Locale.Get("summon.blacklisted", p), p.FormatNick(target));
                return false;
            } else if (result == AccessResult.BelowMinRank) {
                p.Message(Locale.Get("summon.below_min_rank", p),
                ...
                p.Message(Locale.Get("summon.above_max_rank", p),
                          Group.GetColoredName(p.level.VisitAccess.Max),
                          p.FormatNick(target), target.group.ColoredName);
            }

            p.Message(Locale.Get("summon.confirm", p), target.name);
            return false;
        }
        
        public override void Help(Player p) {
            p.Message(Locale.Get("summon.help1", p));
            p.Message(Locale.Get("summon.help2", p));
            p.Message(Locale.Get("summon.help3", p));
            p.Message(Locale.Get("summon.help4", p));
        }
    }
}
