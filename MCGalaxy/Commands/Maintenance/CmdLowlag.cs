    ...
    public sealed class CmdLowlag : Command2 {
        ...
        public override void Use(Player p, string message, CommandData data) {
            if (message.Length == 0 && Server.Config.PositionUpdateInterval > 1000) {
                ...
                Chat.MessageAll(Locale.Get("lowlag.turned_off"));
            } else if (message.Length == 0) {
                Server.Config.PositionUpdateInterval = 2000;
                Chat.MessageAll(Locale.Get("lowlag.turned_on"));
            } else {
                int interval = 0;
                if (!CommandParser.GetInt(p, message, "Interval", ref interval, 20, 2000)) return;

                Server.Config.PositionUpdateInterval = interval;
                Chat.MessageAll(string.Format(Locale.Get("lowlag.update_interval"), interval));
            }
            SrvProperties.Save();
        }

        public override void Help(Player p) {
            p.Message(Locale.Get("lowlag.help1", p));
            p.Message(Locale.Get("lowlag.help2", p));
            p.Message(Locale.Get("lowlag.help3", p));
        }
    }
}
