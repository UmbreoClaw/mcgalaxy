    ...
    public sealed class CmdPay : MoneyCmd {
        ...
        public override void Use(Player p, string message, CommandData data) {
            ...
            if (trans.Reason != null && !p.CanSpeak()) {
                p.Message(Locale.Get("pay.cannot_speak", p));
                return;
            }
            ...
            if (matches > 1) return;
            if (p == who) { p.Message(Locale.Get("pay.no_self_pay", p), Server.Config.Currency); return; }
        ...
        static bool IsLegalPayment(Player p, int payer, int receiver, int amount) {
            if (receiver + amount > 16777215) {
                p.Message(Locale.Get("pay.max_balance", p), Server.Config.Currency); return false;
            }
            if (payer < amount) {
                p.Message(Locale.Get("pay.not_enough", p), Server.Config.Currency); return false;
            }
            return true;
        }
        
        public override void Help(Player p) {
            p.Message(Locale.Get("pay.help1", p));
            p.Message(Locale.Get("pay.help2", p), Server.Config.Currency);
        }
    }
}
